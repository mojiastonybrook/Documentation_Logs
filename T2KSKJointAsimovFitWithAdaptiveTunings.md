...
## General scheme for invloving auto-adaptation feature in MCMC
...
The chain at its first two stages is in the phase of enabling auto-adaptive step size tuning. Generally in this phase, the chains start with their initial setting and run with adaptations on their proposal functions via continuing to update the covariance matrix of the parameters every a few thousands steps according to the accumulated samples. This phase will last until the chains reach or get close to their equilibrium states and the last updated covariance matrix are kept for the third stage.

At the third stage, the chain moves into a different phase where the auto-adaptation is fully turned off. The chains start at where they are stopped at the previous stage; the adapted covariance matrix in the proposal functions are frozen at fixed values during the whole execution of this stage.
...
The auto-adaptation on step sizes are on for `AtmSKDet` (shifting and smearing parameters), `SKCalibration` (supplementary SK-sample-specified uncertainties) and `SKDetBeam` (supplementary T2K-sample-specified uncertainties) with the following commands in the card:
```
// AtmSKDet
...
If `USEADAPTIVE` is set to `false` then the group will use the default step-size setting in MCMC. In that mode there is no adaptation at all in the whole process. 

According to adpative MH algorithm, there should be a modfication to the step size scaling factor for each group, which is related to the size of that group. All the parameters with auto-adaptation on step sizes are formally treated as entries of one "global matrix"; in this case, each group forms a sub-block in that global matrix, uncorrelated to the other blocks. The following commands indicate that the step size scaling factors respective for the above groups are modfied by the size of the same global matrix and the size is `263` as `224 (atmskdet) + 21 (skcalib) + 18 (skdetbeam)`.  
```
...
## adaptation-on Phase
As introduced above, the adaptation on step sizes is turned on for the additional detector uncertainty parameters, i.e. the covaraince matrix used in the proposal function would be constantly updated every a few hundreds or thousands of steps according to the sampled points in the prameter space during that interval. 

In practice, as shown in the scheme diagram, this phase is broken into two stages with different adaptation settings.
### Stage 1
The chain starts with initial setting; the adaptation on the covariance matrix is firstly made after sampling `1000` points and would then be updated once every `100` steps until the total amount of steps generated with adaptation reaches to `100000`, according to the set-up of `Stage 1` job in the batch submitter 
...
Besides setting the adaptive covariance matrix, the mean values of the parameters and the adaptation step counter to `0`, the final saved adaptive covariance matrix from last stage would be read in to form the initial proposal function at ***reset***. 
...
**NOTE:** `lower_adapt` and `upper_adapat` would be referred to `ADAPTIONTHRESHOLDS` in configuration card (example: [https://github.com/mojiastonybrook/MaCh3JobSubmitterForAdaptMC/blob/0c1c48d5140a4826c37e0efe4a46fa8ee68ff50d/AtmConfig_adaptiveMC_temp_GlobalSize.cfg#L45](https://github.com/mojiastonybrook/MaCh3JobSubmitterForAdaptMC/blob/0c1c48d5140a4826c37e0efe4a46fa8ee68ff50d/AtmConfig_adaptiveMC_temp_GlobalSize.cfg#L45)) to control when auto-adaptations happen during MCMC. In MaCh3 [https://github.com/t2k-software/MaCh3/tree/DBarrow_JointFit_AtmDetSyst](https://github.com/t2k-software/MaCh3/tree/DBarrow_JointFit_AtmDetSyst), there is also a counter respective for keeping records of the number of steps with adaptation, `total_steps`, which is defined in `covarianceBase` class ([covarianceBase](https://github.com/t2k-software/MaCh3/blob/DBarrow_JointFit_AtmDetSyst/covariance/covarianceBase.h)). ***Nota bene***, `total_steps` is not always equal to the full amount of steps of the chain, because after resetting adaptation `total_steps` is forced to be `0`, meaning that we basically want to start the adaptation from scratch again, while the acutal amount of the steps in that one chain is not zero since there are some steps accumulated from last job and the chain just reassumes running where it stops.      
